...
const getAll = async (res, query, name, ...args) => {
  const {redis} = res;
  log(`[crudOps.getAll] ${name}`);
  if (res.redis) {
    redis.setValue("CRUD-OPS-REDIS", "awesome!", 360);
  }
  try {
